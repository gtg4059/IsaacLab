...
if TYPE_CHECKING:
    ...
    from .commands_cfg import NormalVelocityCommandCfg, UniformVelocityCommandCfg, UniformVelocityTargetCommandCfg
...
class UniformVelocityTargetCommand(CommandTerm):
    r"""Command generator that generates a velocity command in SE(2) from uniform distribution.
    ...
    cfg: UniformVelocityTargetCommandCfg
    """The configuration of the command generator."""

    def __init__(self, cfg: UniformVelocityTargetCommandCfg, env: ManagerBasedEnv):
        """Initialize the command generator.
    ...
    def _update_command(self):
        ...
        self.vel_command_b[:, 0] = torch.where(vec_norm>0.2,torch.where(torch.cos(self.angle-self.robot.data.heading_w)>0,torch.cos(self.angle-self.robot.data.heading_w)*2.0*torch.tanh(2*vec_norm),0),torch.cos(self.angle-self.robot.data.heading_w)*0.5*torch.tanh(5*vec_norm))
