...
class UniformPoseCommand(CommandTerm):
    ...
    def __init__(self, cfg: UniformPoseCommandCfg, env: ManagerBasedEnv):
        ...
        # print("self.body_idx",self.robot.find_bodies(cfg.body_name)[0][0])
        # print("self.body_idx",self.robot.find_bodies(cfg.body_name)[0][1])
        # create buffers
...
class UniformPoseTrigCommand(CommandTrigTerm):
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        # -- position
        # Initialize tensors for r, theta, z with the given size and device
        r = torch.empty(len(env_ids), device=self.device)
        theta = torch.empty(len(env_ids), device=self.device)
        z = torch.empty(len(env_ids), device=self.device)
        # Generate random values within specified ranges using uniform distribution
        ...
        x = r * torch.cos(theta)
        y = r * torch.sin(theta)
        # Update the pose command tensor for each environment ID
