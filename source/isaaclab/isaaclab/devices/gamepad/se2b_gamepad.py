...
@dataclass
class Se2bGamepadCfg(DeviceCfg):
...
class Se2bGamepad(DeviceBase):
    ...
    def advance(self) -> torch.Tensor:
        ...
        X_value = -1.0 if self._X else 1.0
        Y_value = -1.0 if self._Y else 1.0
        A_value = -1.0 if self._A else 1.0
        B_value = -1.0 if self._B else 1.0
        buttons = np.array([X_value, Y_value, A_value, B_value], dtype=np.float32)
        numpy_result = self._resolve_command_buffer(self._base_command_raw)
        command = np.append(numpy_result, buttons)
