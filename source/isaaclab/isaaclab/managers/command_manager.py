...
class CommandTrigTerm(ManagerTermBase):
    ...
    def __init__(self, cfg: CommandTrigTermCfg, env: ManagerBasedRLEnv):
        ...
        # -- metrics that can be used for logging
        self.metrics = dict()
        # -- resampling trigger range
        # self.resampling_range=cfg.resampling_trigger
        # -- resampling trigger
    ...
    def compute(self, dt: float):
        ...
        # # for metric_name, metric_value in self.metrics.items():
        # #     self.trigger += metric_value
        # #     #print(metric_name)
        
        # for metric_name, metric_value in self.metrics.items():  
        #     self.trigger += metric_value
        #     # out=(metric_value <= self.resampling_range).float()
        #     # self.time_left -= dt*out        
        #     # if metric_value <= self.resampling_range:
        #     #     self.time_left -= dt
        #         # print(self.time_left)
        #         #self.trigger += metric_value
        #     # print(metric_name)

        # self.time_left-=dt*(self.trigger<=self.resampling_range).float()
        # # print(self.time_left,self.trigger)
        # resample_env_ids = (self.time_left <= 0.0).nonzero().flatten()
        # # resample_env_ids = (self.trigger <= self.resampling_range).nonzero().flatten()
        # # print(resample_env_ids)
        # if len(resample_env_ids) > 0:
        #     self._resample(resample_env_ids)
        # self.trigger.zero_()
        # # update the command
