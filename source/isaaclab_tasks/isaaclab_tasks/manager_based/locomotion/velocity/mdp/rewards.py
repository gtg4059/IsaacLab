...
def object_ee_distance(
    ...
    #-0.9818-(-1.2359+0.18) = 
    distance1 = torch.norm(curr_pos_w1 - (des_pos_b+torch.tensor([0.0,0.18,0.0],device="cuda:0").repeat(env.num_envs,1)), dim=1, p=2)# 0.
    #-1.5668-(-1.2359-0.18) = 
    distance2 = torch.norm(curr_pos_w2 - (des_pos_b-torch.tensor([0.0,0.18,0.0],device="cuda:0").repeat(env.num_envs,1)), dim=1, p=2)
