...
def feet_air_time_positive_biped(env, command_name: str, threshold: float, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    reward = torch.where(torch.norm(env.command_manager.get_command(command_name)[:, :2], dim=1) < 0.1,torch.clamp(reward, max=threshold),0)
    # reward = torch.clamp(reward, max=threshold)*torch.norm(env.command_manager.get_command(command_name)[:, :2], dim=1) < 0.1
    # print(torch.norm(env.command_manager.get_command(command_name)[:, :2], dim=1))
    # print("reward:",reward)
    return reward
